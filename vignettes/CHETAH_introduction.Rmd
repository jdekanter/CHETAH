...
CHETAH can be downloaded and installed from Github.  
<div class = "blue"> Note that `devtools::install_github` does not always install dependencies:  
...
BiocManager::install(c('bioDist', 'ggplot2', 'gplots', 'cowplot', 'dendextend', 'corrplot', 'reshape', 'plotly'))

# Load the package
library(CHETAH)
```

## Tumor reference Download
...
<div class = "blue"> CHETAH generates generic names for the intermediate types: "Unknown" for cells that are classified to the very first node, and "Node1", "Node2", etc for the additional nodes </div>
...
1.  `input`  
    + An expression matrix of the input cells  
2.  A reference, one of:
    ...
    ```
    iii) In the case of bulk RNA-seq or micro-array reference data (when only one expression profile per cell type is available) an expression matrix must be provided in `ref_profiles` with the cell types as the column names. Note that CHETAH
    runs in this cases without profile and confidence scores and without intermediate types, as a scRNA-seq reference is needed to calculate these, 
    ```{r}
    str(ref_bulkProfiles) ## ref_profiles
    ```
  
To transform reference data in fromat i (as obtained from e.g. a Seurat^3^ object) to format ii, so only óne reference object is needed, run:
```{r} 
ref_listOfMatrices <- lapply(unique(ref_types), function (x) ref_oneMatrix[ ,names(ref_types)[ref_types == x]])
names(ref_listOfMatrices) <- unique(ref_types)
```
 ...
 * `reference_hn`: A list of expression matrices derived from data from Puram et al. (2017) Cancer Cell.  
    ```{r}
    ls.str(reference_hn)
    ```
 ...
 * Better results can be achieved by using a reference and an input dataset that are from the same biological type, or at least consist of cells that are in the same cell state. E.g. for an input dataset of PBMCs, a bone marrow reference dataset could be used, but as these cells are more naive or precursors, this might negatively influence the classification. In this case, another PBMC dataset would work optimally.
 ...
 * The sparser the reference data, the more reference cells are needed to create a reliable, robust reference profile. For high coverage Smart-Seq2^2^ data, as little as 20-30 cells are needed per cell type. For sparser 10X Genomics data, 100+ cells normally gives optimal results.
  
  
## Calling the function
On the example data, CHETAH is run as follows:
...
This function plots both the classification tree and the t-SNE (or similar coordinates) map.  
...
```
<div class = "blue"> Please note that each type "NodeX" corresponds to the node with number X and that the type "Unknown" corresponds to the node with number 0  </div>
<br>
...
```{r out.width="100%", dpi = 100, fig.height = 6, fig.width = 12}
colors <- PlotCHETAH(chetah = chetah, coor = tsne, return_col = T)
str(colors)
```

## `CHETAHshiny`
The classification of CHETAH and other outputs like profile and confidence scores can be manually
assessed in R, but CHETAH also comes with an shiny application that allows for 
easy and interactive visual analysis of the classification.

The shiny implementation needs three inputs:

 * A chetah object (/list) 
 * The 2D coordinates (e.g. from t-SNE) of the input cells
 * The count matrix of the original data (used for the `input` variable in `CHETAHclassifier`  
  
<div class = "blue"> When the data is large (more than 2000 cells), it is advised to use a sparse Matrix for the count data </div>  
