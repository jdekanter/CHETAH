...
So, to run CHETAH, you will only need: 
  ...
  * a reference dataset, annotated with cell types
    + Both as a `SingleCellExperiment` object
...
# Preparing your data
## Required data
To run CHETAH. If you don't have your data stored as `SingleCellExperiment` objects,
you need 4 things:
...
As an example on how to prepare typical data, we will use melanoma input data from [Tirosh et al.](10.1126/science.aad0501) 
...
CHETAH runs on an input and a reference dataset.
Both should be provided in the format of a `SingleCellExperiment` object,
which is a easy way to store differnt kinds of data together.
...
A `SingleCellExperiment` object holds three things:
...
To store our example data in A `SingleCellExperiment` object, we do: 
...
<div class = "blue"> Note: CHETAH defaults to the first `assay`/`reducedDim` in an object and "celltypes" for the reference. See ?CHETAHclassifier and ?PlotCHETAH on how to change this behaviour. </div>

# Running CHETAH
Now that the data is prepared, running chetah is really easy:
...
```{r out.width="100%", dpi = 100, fig.height = 6, fig.width = 12}
PlotCHETAH(chetah = input_mel, interm = TRUE)
```
...
```{r out.width="100%", dpi = 100, fig.height = 6, fig.width = 12}
colors <- PlotCHETAH(chetah = input_mel, return_col = TRUE)
str(colors)
```
...
```{r out.width="100%", dpi = 100, fig.height = 6, fig.width = 12}
input_mel <- Classify(input_mel, 0.8)
PlotCHETAH(chetah = input_mel)
```
