...
which is an easy way to store differnt kinds of data together.
...
<div class = "blue"> Note: CHETAH unctions default to the first `assay`/`reducedDim` in an object and "celltypes" for the reference's `colData`. See ?CHETAHclassifier and ?PlotCHETAH on how to change this behaviour. </div>
...
## Renaming types
For renaming types in the tree, CHETAH comes witht the `RenameBelowNode` function.
...
To reset the classification to it's default, just run `Classify` again:
