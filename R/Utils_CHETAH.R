...
#'
#' @description CHETAH will hierarchically cluster the reference data
...
#' correlations and calculation of profile and confidence scores. \cr
#' Some input cells will reach the leaf nodes of the ct (the pre-defined cell types),
#' these classifications are called \strong{final types} \cr
...
#' \emph{NOTE: We recommend to use the default parameters,
#' as these usually give the best results.}
#'
...
# Called by the CHETAHclassifier. Determines the branches of the current node of the classifcation tree,
# calls ScoreNode and reruns itself on the next-higher node(s).
# The fact that this function is recursive, makes it possible to do all calculations from top to bottom
SplitNodes <- function (input, Env, ref_profiles, all_ref_cells, ref_types) {
  ...
  ## If a branch contains 2 or more cell types (and thus other nodes), perform the steps in the lower nodes
  if (length(branch1) > 1) {
...
# Does the actual gene selections and correlations and calculates confidence scores.
ScoreNode <- function  (ref_profiles, branch1, branch2, input, Env, all_ref_cells, ref_types) {
  ...
  for(uniquetype in seq_len(length(leaves))) {
    ...
    if (!is.null(all_ref_cells)) {
      ...
      if (Env$subsample) {
        ...
        for(i in seq_len(length(oth_branch))) {
          ...
          if (length(cells) > min_group_size ) cells <- cells[sample(length(cells), min_group_size)]
    ...
    ## select genes that are differentially expressed between current type and the other branch
    if (Env$gs_method == "wilcox" & !is.null(all_ref_cells)) {
      genes <- doWilcox(current_cells = current_cells, otherbranch_c = otherbranch_c,
                        all_ref_cells = all_ref_cells, Env = Env)
    } else {
      genes <- FindDiscrGenes(ref_profiles = ref_profiles, type = current_type,
                              other_branch = oth_branch, Env = Env)
    }
    ...
    if (Env$cor_method == "cosine") {
      correlations <- doCosine(genes = genes, input = input, all_ref_cells = all_ref_cells,
                               current_cells = current_cells, otherbranch_c = otherbranch_c,
                               ref_profiles = ref_profiles, type = current_type)
    } else {
      correlations <- doCorrelation(ref_profiles = ref_profiles, genes = genes, input = input,
                                    all_ref_cells = all_ref_cells,
                                    current_cells = current_cells, otherbranch_c = otherbranch_c,
                                    type = current_type, Env = Env)
    }
    cor_i <- correlations[["cor_i"]] ## correlation of input cells to the current reference profile (RP)
    cor_t <- correlations[["cor_t"]] ## correlation of reference cells of current type to RP
    cor_ob <- correlations[["cor_ob"]] ## correlation of reference cells in the other branch to RP
...
PlotTSNE <- function (toplot, coor, col = NULL, return = FALSE,
  ...
  if(class == "numeric" | class == 'integer') {
    ...
    plot <- plot + scale_color_gradientn(colors = col, limits = limits)
  }

  ## Define plotting limits
  if (!is.null(x_limits)) { plot <- plot + lims(x = c(x_limits[1]-1, x_limits[2]+1)) }
  if (!is.null(y_limits)) { plot <- plot + lims(y = c(y_limits[1]-1, y_limits[2]+1)) }
...
#' @param class a factor of the length of \code{toplot} to indicate the groups
#' @param col a character vector of colors, with the names of the factor levels
#' @param grad_col if defined, the jittered points of
#' the individual values of each cell behind
#' the boxplot will be colored by these gradient colors based on their value
